...
partial model PartialAirside
  ...
  extends dSpaceExample.FiveZones.BaseClasses.PartialPhysicalAirside(heaCoi(
        dp1_nominal=30000));
  ...
  FaultInjection.Utilities.InsertionTypes.Variables.SignalCorruption.Max maxAtt[5](
      faultMode=faultZoneTemp)
    annotation (Placement(transformation(extent={{1122,500},{1144,520}})));
  parameter FaultInjection.Utilities.InsertionTypes.Generic.faultMode faultZoneTemp[5](
    each active=false,
    each startTime=207*24*3600 + 12*3600,
    each endTime=207*24*3600 + 12*3600 + 3*3600,
  ...
  parameter FaultInjection.Utilities.InsertionTypes.Generic.faultMode faultFanSpeed(
    ...
    minimum=0.1,
    maximum=1) "Fan speed control signal blocking control"
    annotation (Placement(transformation(extent={{560,582},{580,602}})));
  FaultInjection.Utilities.InsertionTypes.Variables.SignalCorruption.ExternalInteger
    extAtt(faultMode=faultTZoneRequest)
    annotation (Placement(transformation(extent={{340,400},{360,420}})));
  Buildings.Controls.OBC.CDL.Integers.Sources.Constant attReq(k=15)
    "Attacked request number"
    annotation (Placement(transformation(extent={{300,406},{320,426}})));
  parameter FaultInjection.Utilities.InsertionTypes.Generic.faultMode faultTZoneRequest(
    each active=false,
    each startTime=207*24*3600 + 12*3600,
    each endTime=207*24*3600 + 12*3600 + 3*3600,
    each minimum=273.15 + 15,
    each maximum=273.15 + 28) "Zone temperature reset request attack"
    annotation (Placement(transformation(extent={{340,376},{360,396}})));
equation
  ...
  connect(conAHU.yHea, swiFreSta.u3) annotation (Line(points={{424,554.667},{
          452,554.667},{452,32},{22,32},{22,-108},{10,-108},{10,-138},{18,-138}},
                                                              color={0,0,127}));
  ...
  connect(freSta.y, andFreSta.u1) annotation (Line(points={{22,-92},{28,-92},{
          28,-112},{-40,-112},{-40,-130},{-22,-130}},
                                                 color={255,0,255}));
  connect(flo.TRooAir, maxAtt.u) annotation (Line(points={{1094.14,491.333},{
  ...
  connect(TZonResReq.y, extAtt.u) annotation (Line(points={{322,370},{328,370},
          {328,410},{338,410}}, color={255,127,0}));
  connect(extAtt.y, conAHU.uZonTemResReq) annotation (Line(points={{361,410},{
          376,410},{376,486},{322,486},{322,526.222},{336,526.222}}, color={238,
          46,47}));
