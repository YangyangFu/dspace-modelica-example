...
#define beginJmpParallelSection(i) DYNEXTRABEGIN2 DYNPropagateDidToThread(did_);if (setjmp(DYNGlobal_jmp_buf_env.TheBuffer)==0) {double*duser_=(double*)&DYNB[i];double DYNT=*time;double*time=&DYNT;
...
#define DYNTimeEvent(y,nr,less) \
  (((Init_||DYNEvent) ? DYNTimeEvent2(y,less,(struct BasicDDymosimStruct*)(DSE_STRUCT duser_),DYNTime, ((struct BasicDDymosimStruct*)(DSE_STRUCT duser_))->mOrigTimeError, &AnyEvent_, &AnyDEvent_, PrintEvent&(1 << 1) && (!Init_ || PrintEvent&(1 << 2)), DymolaOneIteration_, &DYNX(QTimed_,nr)):0),DYNX(QTimed_,nr))
/* Deactivate the state event */

#define DYNTimeEventMinor(y,nr,less) \
  (((Init_||DYNEvent) ? DYNTimeEvent2(y,less,(struct BasicDDymosimStruct*)(DSE_STRUCT duser_),DYNTime, ((struct BasicDDymosimStruct*)(DSE_STRUCT duser_))->mOrigTimeError, &AnyEvent_, 0          , PrintEvent&(1 << 1) && (!Init_ || PrintEvent&(1 << 2)), DymolaOneIteration_, &DYNX(QTimed_,nr)):0),DYNX(QTimed_,nr))
/* Deactivate the state event */
