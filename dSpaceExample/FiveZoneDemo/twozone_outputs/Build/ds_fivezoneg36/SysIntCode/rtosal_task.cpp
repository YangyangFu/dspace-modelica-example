...
/* -- Declarations of extern functions. -- */
 ...
 EXTERN_C_DECL void ds_fivezoneg36_DSRTMdlOutputs0(
    void);

 EXTERN_C_DECL void ds_fivezoneg36_DSRTMdlUpdate0(
    void);
    ...
    DsTErrorList ErrorList, DsCTask* pTask) 
{
    ...
    pAppl->errorMessage(RTOSAL_DEADLINE_VIOLATION, "%s%s%s", "ds_fivezoneg36", pTask->getDescription(), CallCounterStr);
    ...
    DsCTask* pTask, int numberOfAcceptedOverruns) 
{
    ...
    if (pTask->getOverrunCounter() > (unsigned int)numberOfAcceptedOverruns)
    {
        ...
        char CallCounterStr[32];
        snprintf(CallCounterStr, 32, "%llu", pTask->getCallCounter());
        pAppl->errorMessage(RTOSAL_TASK_OVERRUN, "%s%s%s%d", "ds_fivezoneg36", pTask->getDescription(), CallCounterStr, numberOfAcceptedOverruns);
    }
